import { AdministratorService, EventBus, TransactionalConnection,Transaction, RequestContext, ID, ProcessContext, ShippingMethodEvent, Administrator } from '@etech/core';
import {
...
@Injectable()
export class ShippingMethodActivityLogService  extends ActivityLogService<ShippingMethodEvent> implements OnApplicationBootstrap{

    async deleteActivityLog(ctx: RequestContext, id: ID): Promise<Boolean> {
        const repo= this.transactionalConnection.getRepository(ctx, ShippingMethodActivityLogEntity);
    ...
    async registerLog(event: ShippingMethodEvent){
        const repo= this.transactionalConnection.getRepository(event.ctx, ShippingMethodActivityLogEntity);
    ...
    async activityLogs(ctx: RequestContext,filter?: ActivityLogFilter):Promise<ActivityLog[]>{
        let allLogsSelectQueryBuilder= await this.transactionalConnection.getRepository(ctx, ShippingMethodActivityLogEntity)
