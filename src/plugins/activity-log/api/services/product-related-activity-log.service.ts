import { AdministratorService, EventBus, ProductEvent, TransactionalConnection,Transaction, Ctx, RequestContext, ID, ProductService, Product, UserService, ProcessContext, ProductOptionGroupChangeEvent, ProductVariantEvent, Administrator } from '@etech/core';
import {
...
@Injectable()
export class ProductRelatedActivityLogService  extends ActivityLogService<ProductEvent|ProductOptionGroupChangeEvent|ProductVariantEvent> implements OnApplicationBootstrap{

    async deleteActivityLog(ctx: RequestContext, id: ID): Promise<Boolean> {
        let repo= this.transactionalConnection.getRepository(ctx, ProductRelatedActivityLogEntity);
    ...
    @Transaction()
    async registerLog(event: ProductEvent|ProductOptionGroupChangeEvent|ProductVariantEvent){
        let repo= this.transactionalConnection.getRepository(event.ctx, ProductRelatedActivityLogEntity);
    ...
    async activityLogs(ctx: RequestContext,filter?: ActivityLogFilter):Promise<ActivityLog[]>{
        let allLogsSelectQueryBuilder= await this.transactionalConnection.getRepository(ctx, ProductRelatedActivityLogEntity)
    ...
    onApplicationBootstrap() {
        if(this.processContext.isServer){
            ...
            this.eventBus.ofType(ProductOptionGroupChangeEvent).subscribe((async (event)=>{
                // console.log('event happened here')
                await this.registerLog(event);
            }));
